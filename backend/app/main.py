...
Endpoints:
  ...
  POST /api/settings/storage-root   { "path": "D:\\ResearchWorkspace" }
  GET  /api/tree
  POST /api/files/save
  POST /api/bookmarks
  GET  /api/files/read
...
class SaveFileBody(BaseModel):
    relative_path: str = Field(
        ..., description="Path relative to storage root, e.g. summaries/notes.md"
    )
    content: str = Field(..., description="File content as UTF-8 text")


class BookmarkBody(BaseModel):
    folder: str = Field("", description="Relative folder under storage root")
    title: str
    url: str
...
def set_storage_root(body: StorageRootBody) -> dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Cannot create folder: {exc}"
        ) from exc
...
def reset_storage_root_default() -> dict[str, str]:
    """Re-run D:-first detection and switch to that location."""
...
def save_bookmark(body: BookmarkBody) -> dict[str, str]:
    """Save a Windows .url Internet Shortcut under the storage root."""
    root = current_root()
